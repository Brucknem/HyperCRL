...
def compute_fisher(task_id, collector, params, device, mnet, hnet=None,
    ...
    Args:
        task_id: The ID of the current task, needed to store the computed
            ...
            embedding.
        collector: A data handler. We will compute the Fisher estimate across the
        ...
        custom_nll (optional): A function handle that can replace the default 
            ...
            sample.
        out_var (optional): whether regression output contain log variance
    """
    ...
    for s, data in enumerate(loader):
        x_t, a_t, T = data
        x_t, a_t, T = x_t.to(device), a_t.to(device), T.to(device)
        ...
        if custom_nll is not None:
            ...
            if out_var:
                mu, logvar = torch.split(Y, Y.size(-1) // 2, dim=-1)
            else:
                mu = Y
                logvar = torch.zeros_like(mu)
            inv_var = torch.exp(-logvar)
            nll = 0.5 * ((mu - T) ** 2) * inv_var + 0.5 * logvar
            nll = nll.sum()
            ...
            # MASTER_THESIS, distinguish between empiricial and normal fisher!
            if empirical_fisher:
                if not time_series:
...
def ewc_register_buffer(mnet, hnet, task_id, online=False):
    ...
    for i, W in enumerate(weights):
        fake_main_params.append(torch.nn.Parameter(torch.zeros(*W.shape),
                                                   requires_grad=True))
        fake_main_params[i].data = weights[i]

    for i, p in enumerate(fake_main_params):
        buff_w_name, buff_f_name = _ewc_buffer_names(task_id, i, online)

        # We use registered buffers rather than class members to ensure that
        # these variables appear in the state_dict and are thus written into
        # checkpoints.
        mnet.register_buffer(buff_w_name, p.detach().clone())

        mnet.register_buffer(buff_f_name, p.detach().clone())
