...
class RandomAgent(Agent):
    ...
    def act(self, state, task_id=None):
        return np.random.randn(len(state), self.control_dim)
...
class MPC(Agent):
    ...
    def _dynamics(self, x, u, task_id):
        ...
        if self.gt_dynamic:
            logging.info((xx_gt - xx).mean(dim=0))
            return xx_gt
        return xx
    ...
    def act(self, state, task_id=None, first_action=True):
        if isinstance(state, np.ndarray) and len(state.shape) >= 2:
            return torch.tensor(np.array(
                [self.act(s, task_id=task_id, first_action=first_action).detach().cpu().numpy() for s in state]))
        self.model.eval()
