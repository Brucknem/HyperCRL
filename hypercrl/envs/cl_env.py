...
class CLEnvHandler():
    ...
    def add_task(self, task_id, render=False, replica=False, hyper_dict=None):
        ...
        if self.cl_env == "lqr":
            ...
            logging.info(env.model.opt.gravity)
        ...
        # For openai GYM environments, we set seed and wrap with monitor
            ...
            env = GymWrapper(env)
        elif self.cl_env.startswith("manipulator_environment"):
            import Utils.env_model
            env = Utils.env_model.get_env(hyper_dict, True)

        if not self.cl_env.startswith("lqr") and not self.cl_env.startswith("manipulator_environment"):
            env.seed(self.seed)
