...
from HyperCRL.hypercrl.model import reload_model, reload_model_hnet
from HyperCRL.hypercrl.envs.cl_env import CLEnvHandler
from HyperCRL.hypercrl.tools import reset_seed, read_hparams
...
def evaluate_reward(env, agent, task_id):
    ...
    logging.info(f"Evaluating {N_run_per_seed} took {(time.time() - ts):.1f} s")
    return avg_rewards
        ...
        for j in range(tasks):
            ...
            for t in range(j + 1):
                ...
                if run_s:
                    ...
                    logging.info(f"{model}, Task{t}-{j}: {rew} / {avgtime}")
                else:
                    rew = evaluate_reward(env, agent, t)
                    logging.info(f"{model}, Task{t}-{j}: {rew}")
                stats[m, t, j] = rew
    return stats
...
def work_single(arg):
    ...
    for t in range(tasks):
        ...
        if run_s:
            rew, avgtime = evaluate_success(env, agent, t)
            logging.info(f"single {rew}/{avgtime}")
        else:
            rew = evaluate_reward(env, agent, t)
        stats[t] = rew
